...
export default class DeviceStorage {
  ...
  static getChart = async (str, navigationIndex, status) => {
    ...
    if (status == 0) {
      ...
      bookArr = bookArr.filter((item, index, array) => {
        const week = DateExtension.getWeek(
          DateExtension.strToDate(item.year, item.month, item.day),
        );
        if (
          data.year == item.year &&
          data.week == week &&
          item.cmodel.is_income == navigationIndex
        ) {
          arr.push(item);
        }
        return true;
    ...
    // 年
      ...
      for (var i = 0; i < bookArr.length; i++) {
        ...
        if (
          ...
          model.cmodel.is_income == navigationIndex
        ) {
          arr.push(model);
        }
      }
    ...
    if (status == 0) {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        chartArrData.push({
          date:
            DateExtension.supplement(date.getMonth() + 1) +
            '-' +
            DateExtension.supplement(date.getDate()),
          price: 0,
        });
      }
  ...
  static getChartDateRange = async (is_income, status) => {
    ...
    // 月
      ...
      for (var i = minDate.getFullYear(); i <= maxDate.getFullYear(); i++) {
        const minMonth =
          i == minDate.getFullYear() ? minDate.getMonth() + 1 : 1;
        const maxMonth =
          i == maxDate.getFullYear() ? maxDate.getMonth() + 1 : 12;
  ...
  static addCusCategory = async (text, model, is_income) => {
    ...
    var cateSysRemovePayArr = await DeviceStorage.load(
      SAVE.PIN_CATE_SYS_REMOVE_PAY,
    );
    var cateCusHasPayArr = await DeviceStorage.load(SAVE.PIN_CATE_CUS_HAS_PAY);
    var cateCusHasPaySyncedArr = await DeviceStorage.load(
      SAVE.PIN_CATE_CUS_HAS_PAY_SYNCED,
    );

    var cateSysHasIncomeArr = await DeviceStorage.load(
      SAVE.PIN_CATE_SYS_HAS_INCOME,
    );
    var cateSysRemoveIncomeArr = await DeviceStorage.load(
      SAVE.PIN_CATE_SYS_REMOVE_INCOME,
    );
    var cateCusHasIncomeArr = await DeviceStorage.load(
      SAVE.PIN_CATE_CUS_HAS_INCOME,
    );
    var cateCusHasIncomeSyncedArr = await DeviceStorage.load(
      SAVE.PIN_CATE_CUS_HAS_INCOME_SYNCED,
    );

    var newmodel = {
      id:
        cateSysHasPayArr.length +
        cateSysRemovePayArr.length +
        cateCusHasPayArr.length +
        cateSysHasIncomeArr.length +
        cateSysRemoveIncomeArr.length +
        cateCusHasIncomeArr.length +
        1,
      name: text,
      icon_n: model.icon_n,
      icon_l: model.icon_l,
  ...
  static getCategorySet = async () => {
    ...
    var payRemove = pay3;
    var pay = [
      {title: 0, data: payInsert},
      {title: 1, data: payRemove},
    ];
    ...
    var income = [
      {title: 0, data: incomeInsert},
      {title: 1, data: incomeRemove},
    ];
